...
const Contact: React.FC<ContactProps> = ({ data }) => {
    ...
    const handleSubmit = (e: React.FormEvent) => {
        ...
        // Simulates a quick processing frame; you can bind a Formspree endpoint link here later
    ...
    return (
        <div className="container w-full mx-auto px-4">
            ...
            <div className="title mb-12">
                <span className="text-xs font-bold uppercase tracking-widest text-sky-500">Inquiries</span>
                <h1 className="text-4xl font-extrabold tracking-tight text-slate-900 mt-1">Get in Touch</h1>
                <p className="text-sm text-slate-500 font-medium mt-1.5">Feel free to reach out for collaborations or project inquiries.</p>
            </div>
            ...
            <div className="grid grid-cols-1 lg:grid-cols-12 gap-12 items-start">
                ...
                <div className="lg:col-span-5 space-y-5">
                    ...
                    <div className="flex items-center gap-4 p-4 rounded-2xl border border-slate-200/80 bg-slate-50/40 hover:bg-white hover:border-sky-500/20 transition-all duration-300">
                        ...
                        <div className="min-w-0 flex-1">
                            ...
                            <p className="text-sm font-semibold text-slate-800 mt-0.5 truncate">
                                <a href={`mailto:${info.email}`} className="text-slate-700 hover:text-sky-600 transition-colors duration-150">{info.email}</a>
                            </p>
                        </div>
                    </div>
                    ...
                    <div className="flex items-center gap-4 p-4 rounded-2xl border border-slate-200/80 bg-slate-50/40 hover:bg-white hover:border-sky-500/20 transition-all duration-300">
                        ...
                        <div className="min-w-0 flex-1">
                            <span className="text-[10px] font-bold uppercase tracking-wider text-slate-400">Location Base</span>
                            <p className="text-sm font-semibold text-slate-700 mt-0.5 truncate">{info.location}</p>
                        </div>
                    </div>
                    ...
                    <div className="flex items-center gap-4 p-4 rounded-2xl border border-slate-200/80 bg-slate-50/40 hover:bg-white hover:border-sky-500/20 transition-all duration-300">
                        ...
                        <div className="min-w-0 flex-1">
                            <span className="text-[10px] font-bold uppercase tracking-wider text-slate-400">Mobile Hub</span>
                            <p className="text-sm font-semibold text-slate-700 mt-0.5 truncate">
                                <a href={`tel:${info.phone}`} className="text-slate-700 hover:text-sky-600 transition-colors duration-150">{info.phone}</a>
                            </p>
                        </div>
                    </div>
                ...
                <div className="lg:col-span-7">
                    <form onSubmit={handleSubmit} className="w-full space-y-6 p-6 rounded-2xl border border-slate-200 bg-slate-50/40 shadow-sm hover:shadow-md transition-shadow duration-300">
